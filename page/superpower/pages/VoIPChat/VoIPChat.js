...
Page({
  ...
  getSignature(nonceStr, timeStamp){
    let sessionKey = this.data.session_key
    
    let signature = CryptoJs.HmacSHA256([appId, groupId, nonceStr, timeStamp].sort().join(''), sessionKey)
      ...
      success: (res)=>{
        if(res.errCode){
          ...
          qq.onVoIPChatMembersChanged((r)=>{
            let newCount = r.openIdList.length
